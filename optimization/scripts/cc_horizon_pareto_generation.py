...
def main(date_str: str, n_parallel_evals: int, base_path: Path, env_path: Path, values_per_decision_variable: int, power_threshold: float, file_id: str):
    ...
    with pd.HDFStore(output_path, mode='w') as store:
        for step_idx, df_paretos, consumption_array in results:
            table_key_base = f"{date_str}_step_{step_idx:03d}"
            store.put(f"/pareto/{table_key_base}", df_paretos)
            store.put(f"/consumption/{table_key_base}", pd.DataFrame(consumption_array, columns=["Cw", "Ce"]))
...
if __name__ == "__main__":
    ...
    file_id = f"cc_horizon_optimization_eval_at_{datetime.datetime.now():%Y%m%dT%H%M}"
