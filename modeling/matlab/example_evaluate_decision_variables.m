...
env_vars.mv_kgh = 133992.0; % 170;  % Steam mass flow rate (kg/h)
env_vars.Tv_C = 41; % 36.0;    % Vapour temperature (°C)

%% Define decision variable arrays (values to test)
dv_values = struct();
dv_values.qc = linspace(1107., 3960., 5); %linspace(6, 24, 5);      % Cooling flow rates (m³/h) - 5 values
...
%     "inverse_wct_model_data_path", "/home/patomareao/development/SOLhycool/modeling/data/models_data/model_data_wct_inverse_fp_pilot_plant_gaussian_mimo.mat"); %"/home/patomareao/development/SOLhycool/modeling/matlab/component_models/model_data_wct_fp_gaussian.mat");
options_struct = struct(...
    "silence_warnings", false, ...
    "resolution_mode", "inverse", ...
    "model_type", "data", ...
    "wct_n_wct", 3, ...
    "wct_lb", [0.1,   0.1,     5.0,      1107.,   0.], ...
    "wct_ub", [50.,   99.99,   55.,      3960.,   95.], ...
    "wct_model_data_path", "/home/patomareao/development/SOLhycool/modeling/data/models_data/model_data_wct_fp_andasol_gaussian_cascade.mat", ...
...
[Ce, Cw, detailed] = combined_cooler_model( ...
    env_vars.Tamb_C, ...
    env_vars.HR_pp, ...
    env_vars.mv_kgh, ...
    qc, ...
...
[Twct_out, Ce, Cw] = wct_model_data(40, env_vars.HR_pp, 40, qc, wwct, ...
    model_data_path=options_struct.wct_model_data_path, ...
    silence_warnings=false, ...
...
if ~isempty(dv_list)
    ...
    for i = 1:n_display
        ...
        fprintf('%-8.2f %-8.2f %-8.2f %-8.2f %-8.2f %-12.2f %-12.2f\n', ...
                ...
                consumption_list{1}, consumption_list{2}(i));
    end
