...
options_struct = struct(...
    ...
    "inverse_wct_model_data_path", "/home/patomareao/development/SOLhycool/modeling/data/models_data/model_data_wct_inverse_fp_pilot_plant_gaussian_mimo.mat",...
    "dc_lb", [3.0      25.0,    0,  0],...
    "dc_ub", [50.0     55.0,    0, 0] ...
); %"/home/patomareao/development/SOLhycool/modeling/matlab/component_models/model_data_wct_fp_gaussian.mat");
...
% options_struct = struct( ...
%     "model_type", "data", ...
%     "dc_lb", [0.1, 25.0, 0.0, 0.0], ...
%     "dc_ub", [50.0, 65.0, 0.0, 0.0], ...
%     "dc_model_data_path", "../data/models_data/model_data_dc_fp_pilot_plant_gaussian_cascade.mat", ...
%     "wct_lb", [0.1, 0.1, 25.0, 1107.0, 20.0], ...
%     "wct_ub", [50.0, 99.9, 65.0, 3960.0, 100.0], ...
%     "wct_model_data_path", "../data/models_data/model_data_wct_fp_andasol_gaussian_cascade.mat", ...
%     "wct_ce_coeffs", [5.35e-05, 0.01149, -0.0141, 0.1433], ...
%     "wct_n_wct", 3, ...
%     "condenser_option", 6, ...
%     "condenser_A", 22502.0, ...
%     "condenser_n_tb", 26973, ...
%     "recirculation_coeffs", [1.011e-10, 3.587e-07, 0.002, 0.08816], ...
...
env_vars = struct();
env_vars.Tamb_C = 14.7;  % Ambient temperature (°C)
env_vars.HR_pp = 76.0;   % Relative humidity (%)
env_vars.mv_kgh = 258; % 99649.89557987738;%142356.9936855391; % 170;  % Steam mass flow rate (kg/h)
env_vars.Tv_C = 38; % 36.0;    % Vapour temperature (°C)

%% Define decision variable arrays (values to test)
dv_values = struct();
% dv_values.qc = linspace(1107., 3960., 5) * options_struct.wct_n_wct; %
...
% dv_values.qc = linspace(1107., 3960., 50) * options_struct.wct_n_wct; %linspace(6, 24, 5);      % Cooling flow rates (m³/h) - 5 values

dv_values.qc = linspace(6, 24, 30);      % Cooling flow rates (m³/h) - 5 values
dv_values.Rp = 1;       % Parallel distribution ratios (-) - 3 values
dv_values.Rs = 1;       % Series distribution ratios (-) - 3 values  
dv_values.wdc = 1;    % DC fan percentages (%) - 4 values
