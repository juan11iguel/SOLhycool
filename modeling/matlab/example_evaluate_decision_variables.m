...
env_vars = struct();
env_vars.Tamb_C = 40.0;  % Ambient temperature (°C)
env_vars.HR_pp = 70.0;   % Relative humidity (%)
env_vars.mv_kgh = 133992.0; % 170;  % Steam mass flow rate (kg/h)
env_vars.Tv_C = 41.5; % 36.0;    % Vapour temperature (°C)

%% Define decision variable arrays (values to test)
dv_values = struct();
% dv_values.qc = linspace(1107., 3960., 5) * options_struct.wct_n_wct; %linspace(6, 24, 5);      % Cooling flow rates (m³/h) - 5 values
