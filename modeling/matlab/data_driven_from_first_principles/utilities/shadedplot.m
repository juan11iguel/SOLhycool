function [ha hb hc] = shadedplot(ax, x, y1, y2, varargin)
...
y = [y1; (y2-y1)]'; 
ha = area(ax, x, y, 'HandleVisibility', 'off');
set(ha(1), 'FaceColor', 'none'); % this makes the bottom area invisible
set(ha, 'LineStyle', 'none');

% plot the line edges
% hold on 
hb = plot(ax, x, y1, 'LineWidth', 1, 'HandleVisibility','off', 'LineStyle','--');
hc = plot(ax, x, y2, 'LineWidth', 1, 'HandleVisibility','off', 'LineStyle','--');
% hold off
...
switch length(varargin)
    case 0
    case 1
        set(ha(2), 'FaceColor', varargin{1}, 'FaceAlpha', 0.5);
    case 2
        set(ha(2), 'FaceColor', varargin{1}, 'FaceAlpha', 0.5);
